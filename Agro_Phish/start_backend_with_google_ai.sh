...
# Скрипт для запуска PhishGuard Backend с Google AI

echo "🛡️  PhishGuard Backend - Запуск сервера с Google AI"
echo "======================================================"
echo ""
...
# Устанавливаем GOOGLE_API_KEY если он не установлен
if [ -z "$GOOGLE_API_KEY" ]; then
    echo "⚠️  GOOGLE_API_KEY не установлен в переменных окружения"
    echo "📝 Используйте: export GOOGLE_API_KEY='ваш_ключ'"
    echo ""
    read -p "Введите ваш Google AI API ключ (или нажмите Enter для пропуска): " api_key
    if [ ! -z "$api_key" ]; then
        export GOOGLE_API_KEY="$api_key"
        echo "✅ GOOGLE_API_KEY установлен"
    else
        echo "⚠️  GOOGLE_API_KEY не установлен. Google AI будет недоступен."
    fi
else
    echo "✅ GOOGLE_API_KEY уже установлен"
fi
echo ""
...
if [ ! -z "$GOOGLE_API_KEY" ]; then
    echo "   Google AI: ✅ Включен"
else
    echo "   Google AI: ⚠️  Не настроен"
fi
echo ""
