...
def get_incidents(limit=1000, offset=0, action=None):
    ...
    try:
        ...
        response = requests.get(f"{API_BASE_URL}/incidents", params=params, timeout=10)
...
def get_incident_stats():
    ...
    try:
        response = requests.get(f"{API_BASE_URL}/incidents/stats", timeout=10)
        response.raise_for_status()
        return response.json()
    except requests.exceptions.RequestException as e:
...
def main():
    ...
    # Фильтр по дате
    if incidents:
        dates = [datetime.fromisoformat(incident.get('timestamp', '').replace('Z', '+00:00')) for incident in incidents if incident.get('timestamp')]
        if dates:
