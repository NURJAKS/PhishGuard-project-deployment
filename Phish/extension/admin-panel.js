...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function initApiBase() {
    API_BASE_URL = await resolveApiBase();
  ...
  function isValidDomainOrIP(input) {
    ...
    // Проверка IPv6 (более полная)
    // Поддерживает полные адреса, сокращенные с ::, и localhost
  ...
  // Определение типа записи (домен или IP)
  ...
  function renderBlacklist(domains) {
    ...
    domains.forEach(domain => {
      ...
      const entryType = getEntryType(domain);
  ...
  addDomainBtn.addEventListener('click', async () => {
    ...
    if (!domain) {
      showMessage('Введите домен или IP-адрес', 'error');
      return;
    }

    // Валидация домена или IP-адреса
    if (!isValidDomainOrIP(domain)) {
      showMessage('Некорректный формат домена или IP-адреса', 'error');
      return;
    }
